...
CREATE TABLE IF NOT EXISTS transaction_categories (
    ...
    name        VARCHAR(50)   NOT NULL UNIQUE              COMMENT ' e.g. incoming_money',
    description VARCHAR(255)  NOT NULL DEFAULT ''          COMMENT 'Human-readable explanation',
    created_at  DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT chk_name CHECK (name IN (
...
-- -------------------------------------------------------------
-- 2. users
--    Shows details about the user that is involved in the transaction
--    
-- -------------------------------------------------------------
CREATE TABLE IF NOT EXISTS users (
    id           INT          UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT 'Surrogate PK',
    phone        VARCHAR(20)  NOT NULL UNIQUE COMMENT ' phone number, e.g. +250780000001',
    display_name VARCHAR(100) NOT NULL DEFAULT '' COMMENT 'Name extracted from SMS body if available',
    created_at   DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT chk_phone_format CHECK (phone REGEXP '^\\+[0-9]{7,15}$')
) COMMENT='Phone number based on user registry';
...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    amount      DECIMAL(15,2) NULL                            COMMENT 'Transacted RWF amount, can be NULL',
    ...
    CONSTRAINT chk_amount_validity CHECK (amount IS NULL OR amount >= 0)
